    ...
    const main = {
        ...
        showModal() {
            ...
            function transformToAssocArray(prmstr) {
                let params = {}, prmarr = prmstr.split("&");
            ...
            let colours = ["#1abc9c", "#2ecc71", "#3498db", "#9b59b6", "#34495e", "#16a085", "#27ae60", "#2980b9", "#8e44ad", "#2c3e50", "#f1c40f", "#e67e22", "#e74c3c", "#ecf0f1", "#95a5a6", "#f39c12", "#d35400", "#c0392b", "#bdc3c7", "#7f8c8d"],
                storage = new CrossStorageClient('http://auth.kabeersnetwork.rf.gd/widget/hub.php');
            storage.onConnect().then(function () {
                ...
                if (sessionStorage.getItem('k_logged_in_session') === null) {
                    if (accounts) {
                        fetch('https://hosted-kabeersnetwork.000webhostapp.com/auth/api/widget/api/getClientInfo.php?client_secret=' + main.client.id, {mode: 'cors'})
                            ...
                            .catch(function (error) {
                                console.error('K Auth SDK Failed to Get Client Token, Check Your Connection', error)
                            });
                        for (let i = 0; i < accounts.length; i++) {
                            html += `<div class="k_chooser_card k_chooser_ripple"><img alt="Account Image" class="k_chooser_img" src="https://ui-avatars.com/api/?color=FFFFFF&background=${colours[Math.floor(Math.random() * colours.length)].substr(1)}&format=png&size=250&rounded=true&length=2&name=${accounts[i].username}"><span class="k_chooser_span">${accounts[i].username}<br><span class="k_chooser_email">${accounts[i].email}</span></span></div>`;
                        }
                    }
            ...
            for (let i = 0; i < k_chooser_ripple_rippleElements.length; i++) {
                k_chooser_ripple_rippleElements[i].addEventListener('click', function (e) {
                    let X = e.pageX - this.offsetLeft,
                        ...
                        k_chooser_ripple_rippleDiv = document.createElement("div");
